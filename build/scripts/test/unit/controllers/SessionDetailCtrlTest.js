describe("SessionDetailCtrlTest", function () {
    var $httpBackend;
    var $controller;
    var dataService;
    var $routeParams;
    ...
    describe("Controller session", function () {
        ...
        lapTime = new app.domain.LapTime(1, 444, new Date(2015, 12, 22), 222, 'fred', 111, false);
        lapTimes.push(lapTime);
        lapTime = new app.domain.LapTime(1, 666, new Date(2015, 12, 22), 222, 'fred', 222, false);
        lapTimes.push(lapTime);
        session = new app.domain.Session(1, "Big Red Car", "Long Track", 1, lapTimes);
        beforeEach(function () {
            scope = {};
            $routeParams.sessionId = 1;
            controller = $controller("SessionDetailCtrl", { $scope: scope, dataAccessService: dataService, $routeParams: $routeParams });
            $httpBackend.expectGET("https://t1nj5fds0c.execute-api.ap-northeast-1.amazonaws.com/develop/session/1").respond(session);
        });
...
//# sourceMappingURL=SessionDetailCtrlTest.js.map
